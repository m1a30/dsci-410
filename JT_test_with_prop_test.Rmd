...
```{r visualizing the differences to see ordered trend}
library(ggplot2)

# Convert Age.Groups to a factor with custom order
CAHOOTS_eugene$Age.Groups <- factor(CAHOOTS_eugene$Age.Groups, levels = unique(CAHOOTS_eugene$Age.Groups))

# Create a bar plot
ggplot(data = CAHOOTS_eugene, aes(x = Age.Groups, y = Age.Group.Totals)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(x = "Age Groups", y = "Group Totals", title = "Group Totals by Age Groups") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))


```

```{r visualizing the differences to see ordered trend for ACS}
library(ggplot2)

# Convert Age.Groups to a factor with custom order
ACS_eugene$Age.Groups <- factor(ACS_eugene$Age.Groups, levels = unique(ACS_eugene$Age.Groups))

# Create a bar plot
ggplot(data = ACS_eugene, aes(x = Age.Groups, y = Age.Group.Totals)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(x = "Age Groups", y = "Group Totals", title = "Group Totals by Age Groups") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
data_melted_eug$Age_Groups.Age.Groups <- factor(data_melted_eug$Age_Groups.Age.Groups, 
...
, y = Proportion, fill = Group)) + 
  ...
  labs(title = "Eugene Proportions of Age Groups in ACS and CAHOOTS", 
       x = "Age Groups", 
       y = "Proportion") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  scale_fill_manual(values = c("Proportion_ACS" = "blue", "Proportion_CAHOOTS" = "red"))

```

```{r visualizing the differences to see ordered trend cahoots, springfield}
library(ggplot2)

# Convert Age.Groups to a factor with custom order
CAHOOTS_springfield$Age.Groups <- factor(CAHOOTS_springfield$Age.Groups, levels = unique(CAHOOTS_springfield$Age.Groups))

# Create a bar plot
ggplot(data = CAHOOTS_springfield, aes(x = Age.Groups, y = Age.Group.Totals)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(x = "Age Groups", y = "Group Totals", title = "CAHOOTS Group Totals by Age Groups Springfield") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Convert Age.Groups to a factor with custom order
ACS_springfield$Age.Groups <- factor(ACS_springfield$Age.Groups, levels = unique(ACS_springfield$Age.Groups))

# Create a bar plot
ggplot(data = ACS_springfield, aes(x = Age.Groups, y = Age.Group.Totals)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(x = "Age Groups", y = "Group Totals", title = "Springfield ACS Group Totals by Age Groups") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r stat test for proportions}
library(stats)
prop.test(only_props_sp)
...
, y = Proportion, fill = Group)) + 
  geom_bar(stat = "identity", position = position_dodge(width = 1.2), color="black") + 
  labs(title = "Springfield Proportions of Age Groups in ACS and CAHOOTS", 
       x = "Age Groups", 
       y = "Proportion") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  scale_fill_manual(values = c("Proportion_ACS" = "blue", "Proportion_CAHOOTS" = "red"))
